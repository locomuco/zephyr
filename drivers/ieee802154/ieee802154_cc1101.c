...
#define CC1101_IOCFG2	0x29
#define CC1101_IOCFG1	0x2E
...
static void enable_gpio0_interrupt(struct cc1101_context *cc1101, bool enable)
{
	if (enable) {
		gpio_pin_enable_callback(
			cc1101->gpios[CC1101_GPIO_IDX_GPIO0].dev,
			cc1101->gpios[CC1101_GPIO_IDX_GPIO0].pin);
	} else {
...
static void setup_gpio_callback(struct device *dev)
{
	...
	gpio_init_callback(&cc1101->rx_tx_cb, gpio0_int_handler,
...
static inline bool get_rx_bytes(struct cc1101_context *ctx)
{
	u8_t rx_bytes;
					  ...
					  u8_t pkt_len)
{
	...
	if (pkt_len < 3 ||
	    rx_bytes > (pkt_len + CC1101_FCS_LEN)) {
		return false;
	}
...
static inline bool verify_crc(struct cc1101_context *ctx, struct net_pkt *pkt)
{
	u8_t fcs[2];
...
static void cc1101_rx(struct device *dev)
{
	...
	while (1) {
		...
		if (get_status(cc1101) == CC1101_STATUS_RXFIFO_OVERFLOW) {
		     ...
		     struct net_buf *frag)
{
	...
	if (!write_txfifo(cc1101, &len, CC1101_PHY_HDR_LEN) ||
	    ...
	    get_rx_bytes(cc1101) != (len + CC1101_PHY_HDR_LEN)) {
...
static int cc1101_start(struct device *dev)
{
	...
	enable_gpio0_interrupt(cc1101, true);
...
static int cc1101_stop(struct device *dev)
{
	...
	enable_gpio0_interrupt(cc1101, false);
...
static int power_on_and_setup(struct device *dev)
{
	...
	if (!write_reg_iocfg2(cc1101, CC1101_IOCFG2) ||
	    !write_reg_iocfg1(cc1101, CC1101_IOCFG1) ||
	    !write_reg_iocfg0(cc1101, CC1101_IOCFG0)) {
